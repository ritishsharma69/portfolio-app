...
export default function ProjectDetail() {
  ...
  useEffect(() => {
    if (slug) {
      getProject(slug).then((p) => setProject(p)).catch(() => setProject(null));
    }
  }, [slug]);

  const title = project?.title || (slug || '').replace(/-/g, ' ').replace(/\b\w/g, (s) => s.toUpperCase()) || 'Project';
  ...
  return (
    <Box>
      ...
      <Typography color="text.secondary" sx={{ mb: 3 }}>{project?.shortDesc || `A concise case study of ${title} — goals, approach, and outcomes.`}</Typography>
      ...
      <Stack spacing={3}>
        ...
        <Box>
          ...
          <Stack direction="row" spacing={1} flexWrap="wrap">
            {(project?.tech || ['React','TypeScript','Node.js','MongoDB','Vite','MUI']).map(t => <Chip key={t} label={t} />)}
          </Stack>
        </Box>
        <Box>
          <Typography variant="h6" fontWeight={800} gutterBottom>Key Features</Typography>
          <Stack component="ul" sx={{ m: 0, pl: 3 }}>
            {(project?.features || ['Responsive UI','Authentication','Payments','Analytics','CI/CD']).map(f => <li key={f}><Typography>{f}</Typography></li>)}
          </Stack>
        </Box>
        <Divider />
